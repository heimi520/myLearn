...
def get_sub(codeclass,cname,country_code):
    ...
    text_class=res.json()
    class_pd=pd.DataFrame(text_class)
    class_pd.columns=['code','name','col1','col2']
    ...
    for k,line in enumerate(text_class):
        ...
        fatherCode=line[0]
        is_end=line[-2]
        sub2=get_code_sub(fatherCode,cname,is_end,country_code)
        for code,is_end ,name in sub2[['code','col1','name']].values:
            sub3=get_code_sub(code,name,is_end,country_code)
            for code,is_end ,name in sub3[['code','col1','name']].values:
                sub4=get_code_sub(code,name,is_end,country_code)
          
                ret_list.append(sub4)
         
    ret_pd=pd.concat(ret_list,axis=0)    
    ret_pd['classname']=cname       
    ret_pd['countryCode']=country_code
    return ret_pd
  


def get_code_sub(fatherCode,name,is_end,country_code):
    if is_end=='false':
    ...
    #                'Cookie': 'JSESSIONID=CAA29D75978C61785051A380E997DFC6; insert_cookie=40867542',
        ...
        code_pd['classCode']=name
        code_pd['len']=code_pd['code'].apply(lambda x:len(x))
#        time.sleep(0.2)
    else:
        code_pd=pd.DataFrame([[fatherCode,name]],columns=['code','name'])
        code_pd['classCode']=name
        code_pd['len']=code_pd['code'].apply(lambda x:len(x))
...
def crawl_country_data(country_code,country_name):
    ...
    ret_list=[]
    for k,(codeclass ,cname) in enumerate(code_list):
        print('%s/%s'%(k,len(code_list)), codeclass,cname)
        sub_pd=get_sub(codeclass,cname,country_code)
        sub_pd.to_excel('../data/output/china_%s_%s_v2.xlsx'%(cname,k),index=False,encoding='gbk')
...
spyder_pd=spyder_pd[spyder_pd['code']=='CN']
    
for (country_code,country_name) in spyder_pd[['code','name']].values.tolist():
    print(country_code,country_name)
    try:
        crawl_country_data(country_code,country_name)
    except Exception as e:
        print(country_code,name,e)
#    break    
