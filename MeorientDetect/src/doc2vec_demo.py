...
def get_datasest():
    with open("../data/input/wangyi_title.txt", 'r') as cf:
        docs = cf.readlines()
        print(len(docs))
...
def train(x_train, vector_size=200, epoch_num=1):
    model_dm = Doc2Vec(x_train,min_count=1, window = 3, vector_size = vector_size, sample=1e-3, negative=5, workers=8)
    model_dm.train(x_train, total_examples=model_dm.corpus_count, epochs=2)
