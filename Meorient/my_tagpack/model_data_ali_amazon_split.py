...
rename_map={'Towel Bars':'Towel Racks',
            'Towel Rings':'Towel Racks',
            'Garage Door Openers':'Automatic Door Operators',
            'Gate Operators':'Automatic Door Operators',
            'Bedroom Sets':'Home Bedroom Sets',

              }


data0=pd.read_csv('../data/input/tagpack_orig_data.csv')
...
reg_me=pd.read_excel('../data/tagpack/8.8机器打标标签规则.xlsx').fillna('')
cols=[ 'Product Tag', 'Tag 1', 'Tag 2', 'Tag 3','except']
reg_me=reg_me[cols]
reg_me.columns=['PRODUCT_TAG_NAME','tag1','tag2','tag3','except']


reg=pd.read_excel('../data/tagpack/8.8机器打标标签规则v2.xlsx',sheetname=1)
...
reg=reg[['PRODUCT_TAG_NAME','tag1','tag2','tag3','except']]
...
if 1==1:
    ...
    ok_other.to_csv('../data/input/tagpack_ok_other.csv',index=False)
    
    
#    data_ok2=ok_other[ok_other['sample_w']==1]
#    cols_list=['PRODUCT_NAME','T1','T2','PRODUCT_TAG_NAME','sample_w', 'source']
#    data_ok2=data_ok2[cols_list]


#    aa=data_ok2.groupby('PRODUCT_TAG_NAME')['T1'].count()
