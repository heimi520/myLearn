...
data0=pd.read_csv('../data/tagpack_all/data_orig.csv')
datafilter=data0[data0['T1']!='T1_Other']

reg=pd.read_csv('../data/tagpack_all/reg_merge.csv').fillna('')
...
#
#
#tag_test='Cleaning Brushes'
##tag_test='Mattress Cover'
#aa=data[data['PRODUCT_TAG_NAME']==tag_test]
#
#bb=reg[reg['PRODUCT_TAG_NAME']==tag_test]
...
if 1==1:
    ...
    ok_other.to_csv('../data/tagpack_all/data_filter.csv',index=False)
...
#    aa_count[aa_count['count']<500].to_csv('../data/output/data_not_enough.csv')
    ...
    bad0=aa_cc[aa_cc['count_y']<=100]
    bad=bad0[['tag_x']]
    bad.columns=['PRODUCT_TAG_NAME']
    bad['flag']=1
    bad.index=range(len(bad))
    
    reg_sub=pd.merge(reg,bad,on=['PRODUCT_TAG_NAME'],how='left')
    idx=reg_sub['flag']==1
    tag_new=reg_sub['tag1']+','+reg_sub['tag2']
    reg_sub.loc[idx,'tag1']=''
    reg_sub.loc[idx,'tag2']=''
    reg_sub.loc[idx,'tag1']=tag_new
#    reg_sub.to_csv('../data/tagpack_all/reg_my_v2.csv',index=False)
#    
