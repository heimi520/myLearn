...
if 1==0:    
    ...
    other.to_csv('../data/input/my_other_v2.csv',index=False)
    ...
    for k,v in   enumerate(data.groupby('PRODUCT_TAG_NAME')):
        ...
        voc_pd['pct']=voc_pd['count'].cumsum()/voc_pd['count'].sum()
        voc_pd=voc_pd[voc_pd['pct']<=0.6]
        voc_pd.index=range(len(voc_pd))
    #    voc_dict[v[0]]=voc_pd.copy()
        
        md=td.values.tolist()
        all_list.extend(md)
        num_last=min(max(len(md)*2,500),3000)-len(td)
    
        line=md[0]     
        line_new=line.copy()
        voc_list=voc_pd['key'].tolist()
        for v in range(num_last):
            head_list=voc_list[:4]
            mid_list=voc_list[4:10]
            tail_list=voc_list[10:]
            random.shuffle(head_list)
            random.shuffle(mid_list)
            random.shuffle(tail_list)
            line_1=head_list[:random.randint(2,2)]
            line_2=mid_list[:random.randint(1,2)]
            line_3=tail_list[:random.randint(1,5)]
            line=line_1+line_2+line_3
            random.shuffle(line)
            sentence_new=' '.join(line)
    #        print(sentence_new)
    ...
    data2.to_csv('../data/input/data_add_v2.csv',index=False)


data2=pd.read_csv('../data/input/data_add_v2.csv')
other=pd.read_csv('../data/input/my_other_v2.csv')
other['T1']='T1_Other'
#other=get_other_data()
total_data=pd.concat([data2[cols_list],other[cols_list]],axis=0)
...
#
#d_dict={}
#for v in data2.groupby('PRODUCT_TAG_NAME'):
#    d_dict[v[0]]=v[1]
...
#data['text']=text2feature.text_clean(data['PRODUCT_NAME'])
