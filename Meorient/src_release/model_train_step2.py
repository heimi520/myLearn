...
data0=pd.concat([data_his,data_test],axis=0)
data0['PRODUCT_TAG_NAME']=data0['PRODUCT_TAG_NAME'].map(tag_dict)
data0['PRODUCT_TAG_ID']=data0['PRODUCT_TAG_NAME'].copy()
data_2=data0[data0['PRODUCT_TAG_NAME']=='Ethnic Clothing']


data=pd.read_csv('../data/input/data_step2.csv')
data_1=data[data['PRODUCT_TAG_NAME']!='Ethnic Clothing']
...
def get_other_data():
    ...
    line_list=random.sample(line_list,6000)
...
model.build_model()
model.train(x_train_padded_seqs, y_train,x_val_padded_seqs, y_val,w_sp_train=None) ##data_train['sample_w'].values
