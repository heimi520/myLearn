...
def read_data():
    ...
    ok_other=ok_other.rename(columns={'PRODUCT_TAG_NAME':'PRODUCT_TAG_NAME_FILTER','tag_orig':'PRODUCT_TAG_NAME'})
    ...
    data_new=pd.merge(ok_other,reg[['PRODUCT_TAG_NAME','flag']],on=['PRODUCT_TAG_NAME'],how='left')
    
    data=data_new[data_new['flag'].notnull()]
    data_other=data_new[data_new['flag'].isnull()]
...
cols_list=['PRODUCT_NAME','T2','T1','PRODUCT_TAG_NAME','sample_w', 'source','PRODUCT_TAG_NAME_FILTER']
data=data[cols_list]
...
ok=data_test[data_test['ok']==1]


aa=ok.groupby('PRODUCT_TAG_NAME')['T1'].count()
...
aa=data_test[idx_bad]
aa2=aa[aa['T1']!='Other_T1']

data_step1.to_csv('../data/input/tagpack_ok_other_orig_step1.csv',index=False)
