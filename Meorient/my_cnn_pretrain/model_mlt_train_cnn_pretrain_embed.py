...
##        model=word2vec.Word2Vec.load(os.path.join(self.data_model_dir,'word2vec'))
#        logger.info('loading embed...............')
#        word_vectors = KeyedVectors.load_word2vec_format("../data/word2vec/GoogleNews-vectors-negative300.bin", binary=True)  # C bin format 
#        word_index=self.tokenizer.word_index
#        NUM_WORDS=self.max_words
#        EMBEDDING_DIM=self.voc_dim
#        vocabulary_size=min(len(word_index)+1,NUM_WORDS)
#        embedding_matrix = np.zeros((vocabulary_size, EMBEDDING_DIM))
#        for word, i in word_index.items():
#            if i>=NUM_WORDS:
...
#            except KeyError:
#                embedding_matrix[i]=np.random.normal(0,np.sqrt(0.25),EMBEDDING_DIM)
#        np.savetxt(os.path.join(self.data_model_dir,'embed.csv'),embedding_matrix)
