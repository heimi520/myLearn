...
data0=pd.read_csv('../data/tagpack0830/data_orig.csv')
...
reg0=reg0.fillna('')
reg0=reg0[reg0['PRODUCT_TAG_NAME2']!='']
...
if 1==1:
    ...
    data['PRODUCT_TAG_NAME']=data['PRODUCT_TAG_NAME'].map(reg0.set_index('PRODUCT_TAG_NAME')['PRODUCT_TAG_NAME2'].to_dict())
    

    data_ok=data[data['sect_flag']==1]
    data_ok['sample_w']=1
    
    a=reg0.set_index('PRODUCT_TAG_NAME')['PRODUCT_TAG_NAME2'].to_dict()
    
    
    
#    len(set(data_ok['PRODUCT_TAG_NAME']))
    ...
    ok_other.to_csv('../data/tagpack0830/data_filter.csv',index=False)
