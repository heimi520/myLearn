...
class TextCNN(baseModel):
    ...
    def create_conv_pool(self,conv,conv_layers):
        """
        """
...
#        self.save_obj(tag_id2name_dict,self.tag_id2name_dict_name)        
    ...
    def fit_transform(self,data):
        ...
        tokenizer=Tokenizer(num_words=None, filters='',lower=True,split=' ',char_level=self.is_use_char,oov_token='UNK') 
        ...
        max_words=len(voc_pd[voc_pd['count']>=100 if self.is_use_char else 5 ])  ###filter low frequent words
        logger.info('max_words ////////////////////%s'%max_words)
        
        tokenizer=Tokenizer(num_words=max_words, filters='',lower=True,split=' ',char_level=self.is_use_char,oov_token='UNK') 
        tokenizer.fit_on_texts(data['text'])
        ...
        padded_seqs=pad_sequences(word_ids,maxlen=self.seq_len,padding='post') #将超过固定值的部分截掉，不足的在最前面用0填充
