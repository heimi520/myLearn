...
#    
#    data=pd.read_excel('../data/meorient_data/data_meorient_select_ok_bad.xlsx')
...
data_test=data_test.rename(columns={'T1_NAME_PRED1':'T1_NAME_PRED_BACKUP','TAG_NAME_PRED1':'TAG_NAME_PRED_BACKUP','T2_NAME_PRED':'T2_NAME_PRED_BACKUP'})
...
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/apperel_3c_无法映射买家部分-翻译_buy_%s.xlsx'%T_LEVEL_USED,encoding='gbk',index=False)
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/apperel_3c_付费买家数据翻译后打标_buy_%s.xlsx'%T_LEVEL_USED,encoding='gbk',index=False)
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/apparel_3c_付费买家跑模型-sa_buy_%s_%s.xlsx'%(T_LEVEL_USED,pd.datetime.now().strftime('%Y%m%d')),encoding='gbk',index=False)

#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/apparel_3c_巴西自取买家翻译+跑模型_buy_%s_%s.xlsx'%(T_LEVEL_USED,pd.datetime.now().strftime('%Y%m%d')),encoding='gbk',index=False)
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/apparel_3c_供应商映射标签（截止20190916）_sell_%s_%s.xlsx'%(T_LEVEL_USED,pd.datetime.now().strftime('%Y%m%d')),encoding='gbk',index=False)
...
#data_test.to_excel('../data/output/tag_sell_T0_pred_apparel_3c_adj.xlsx',encoding='gbk',index=False)
...
#
#
#aa=data_test[data_test['PRODUCT_NAME']=='CATV Outdoor Tap (Trunk)']
#
#
#data_test.to_csv()
#
