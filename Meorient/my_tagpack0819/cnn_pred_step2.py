...
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/0819_无法映射买家部分-翻译_buy_%s.xlsx'%T_LEVEL_USED,encoding='gbk',index=False)

#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/0819_付费买家数据翻译后打标_buy_%s.xlsx'%T_LEVEL_USED,encoding='gbk',index=False)
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/0819_付费买家数据翻译后打标_buy_%s.xlsx'%T_LEVEL_USED,encoding='gbk',index=False)
#data_test[data_test['TAG_NAME_PRED']!='Other_TAG'].to_excel('../data/output/0819_付费买家跑模型-sa_buy_%s_%s.xlsx'%(T_LEVEL_USED,pd.datetime.now().strftime('%Y%m%d')),encoding='gbk',index=False)
...
#ret_show.to_excel('../data/output/0819_买家全行业映射标签（20190717_%s_pred_%s.xlsx'%(BUYSELL,T_LEVEL_USED),index=False,encoding='gbk')
#
#
