...
data_add=pd.read_csv('../data/input/tagpack_add_data_step2.csv')

data_add.groupby('sample_w')['T1'].count()

#aa=data_add[data_add['sample_w']>=0.9].groupby('PRODUCT_TAG_NAME')['T1'].count().sort_values().to_frame('count')

data_other=pd.read_csv('../data/input/tagpack_otherdata_step2.csv')
data_other['sample_w']=0.8


data=pd.concat([data_add,data_other],axis=0)


print(data.groupby('sample_w')['T1'].count())
