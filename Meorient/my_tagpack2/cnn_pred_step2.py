...
def read_buy_data():
    ...
    tag_stand=pd.read_excel('../data/tagpack2/8.8机器打标目标标签.xlsx',encoding='gbk')
    tag_stand.columns=['PRODUCT_TAG_NAME','T1','T2']
...
#ret_show[cols].to_csv('../data/output/tagpack2_meoreint_pred_TAG.csv',index=False)
...
ret_show.to_excel('../data/output/tagpack2_meoreint_pred_TAG_classweight_buy_step2.xlsx',index=False,encoding='gbk')
data_test.to_excel('../data/output/tagpack2_meoreint_pred_TAG_classweight_buy_step2.xlsx',index=False,encoding='gbk')
...
bb=data_test[['TAG_NAME_PRED','T1_NAME_PRED','PRODUCT_NAME']].drop_duplicates()
bb.to_csv('../data/output/tagpack2_unique_pred.csv',index=False)
