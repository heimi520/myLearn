...
tag_part=pd.merge(tag,part,on=['T1'],how='inner')
tag_part['part']=1

#list(set(tag['T1']))

md=pd.merge(data,tag,on=['PRODUCT_TAG_NAME'],how='inner')

md_part=pd.merge(md,tag_part[['PRODUCT_TAG_NAME','part']],on=['PRODUCT_TAG_NAME'],how='left')
idx_part=md_part['part']!=1
md_part.loc[idx_part,'T1']='T1_Other'
md_part.loc[idx_part,'T2']='T2_Other'
#md.sample(10000).info()

#md_part['T1']=md_part['T1'].fillna('T1_Other')





#md_part.sample(10000).info()
#
#aa=md.sample(1000)
#
#
cols_list=['PRODUCT_TAG_NAME','T1', 'T2','source','PRODUCT_NAME']
md_part[cols_list].to_csv('../data/part01/data_orig.csv',index=False)

aa=md_part.groupby('T1')[['PRODUCT_TAG_NAME']].count().reset_index().sort_values('T1')
##
#aa=md.groupby('PRODUCT_TAG_NAME')[['T1']].count().reset_index().sort_values('T1')
#aa=aa[aa['T1']<1000]
#aa.columns=['PRODUCT_TAG_NAME','sample_count']
#aa.index=range(len(aa))
#aa.to_csv('../data/tagpack_all/data_not_enough.csv',index=False)
#
