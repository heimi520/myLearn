...
#            embed_w=np.loadtxt(os.path.join(self.data_model_dir,'embed.csv'),'float32')
        text_embed = keras.layers.Embedding(self.max_words+1, ###self.vocab_len + 1,
                                       self.voc_dim ,
        ...
        forward = LSTM(self.rnn_units, return_sequences=True)(l_embedding)  # See equation (1)
        backward = LSTM(self.rnn_units, return_sequences=True, go_backwards=True)(r_embedding)  # See equation (2)
        ...
        semantic = Conv1D(self.filter_num, kernel_size=1, activation="relu")(together)  # See equation (4).
        net = Lambda(lambda x: K.max(x, axis=1))(semantic)  # See equation (5).
