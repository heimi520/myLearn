...
class TextCNN(object):
    ...
    def __init__(self,seq_len=None,max_words=None,\
                 voc_dim=None,num_classes1=None,num_classes2=None, cut_list=[],\
...
#    
    ...
    def build_model(self):
        ...
        self.model = keras.models.Model(inputs=place_x, outputs=[output1_1,output1_2,output2])
        self.model.summary()
        
        opt=keras.optimizers.Adam(lr=self.init_learn_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-08,clipvalue=1.0)
        self.model.compile(loss={'output1_1': 'categorical_crossentropy','output1_2': 'categorical_crossentropy', \
                                 'output2': 'categorical_crossentropy'},\
                                   optimizer=opt,\
                                   metrics=['accuracy'],\
                                   loss_weights={'output1_1': 1,'output1_2':1, 'output2': 1} \
                                   )
    ...
    def train(self,x_train_padded_seqs, y1_train,y2_train, x_val_padded_seqs, y1_val,y2_val,w_sp_train=None):
        ...
        earlystop=EarlyStopping(monitor='val_output2_loss', patience=self.early_stop_count,mode='min', verbose=2)
        callback_lists=[tensorboard,checkpoint,earlystop,monitor]
        ##########################################################
        self.history=self.model.fit(
                                    x={'x_input':x_train_padded_seqs}, 
                                    y={'output1_1':y1_train,'output1_2':y1_train,'output2':y2_train}, 
                                    batch_size=self.batch_size,              
                                    initial_epoch=0,
                                    epochs=self.epoch_max, 
                                    validation_data = (x_val_padded_seqs, [y1_val,y1_val, y2_val]),
                                    verbose=0,
                                    callbacks=callback_lists,
                                    sample_weight=[w_sp_train,w_sp_train,w_sp_train],##3 output
                                    shuffle=True,
