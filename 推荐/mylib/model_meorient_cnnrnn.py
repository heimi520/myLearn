...
#            embed_w=np.loadtxt(os.path.join(self.data_model_dir,'embed.csv'),'float32')
        ...
        conv = Conv1D(128, kernel_size=2, padding="valid")(embed)
        conv=LeakyReLU(alpha=0.05)(conv)
        
        pool = MaxPooling1D(pool_size=2)(conv)

        gru = GRU(self.rnn_units, return_sequences=True,dropout=0.2, recurrent_dropout=0.2)(pool)
        pool_gru = GlobalMaxPooling1D()(gru)
        
        net= Dense(256)(pool_gru)
