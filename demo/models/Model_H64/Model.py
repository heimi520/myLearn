...
class Model(ModelBase):
    ...
    def onInitialize(self):
        ...
        print('decoder_scr summary/////')
        self.decoder_src.summary()
        
        
        print('decoder_dst summary////')
        ...
        rec_src_bgr, rec_src_mask = self.decoder_src( self.encoder(input_src_bgr) )  ###tensor
        rec_dst_bgr, rec_dst_mask = self.decoder_dst( self.encoder(input_dst_bgr) )  ###tensor
        
    
        print('model/////',Model)  ####class 'keras.engine.training.Model'
        ...
        self.ae.compile(optimizer=Adam(lr=5e-5, beta_1=0.5, beta_2=0.999), 
                        loss=[ 
                        DSSIMMSEMaskLoss(input_src_mask, is_mse=self.options['pixel_loss']), 
                        'mae',
                        DSSIMMSEMaskLoss(input_dst_mask, is_mse=self.options['pixel_loss']), 
                        'mae'  ] 
                        )
        print('last model summary///////')
        self.ae.summary()
        ...
        if self.is_training_mode:
            ...
            print('self.training_data_src_path',self.training_data_src_path)
            print('sort_by_yaw_target_samples_path',self.training_data_dst_path if self.sort_by_yaw else None)
    ...
    def onTrainOneIter(self, sample, generators_list):
        ...
        total, loss_src_bgr, loss_src_mask, loss_dst_bgr, loss_dst_mask = self.ae.train_on_batch( [warped_src, target_src_full_mask, warped_dst, target_dst_full_mask], 
                                                                                                  [target_src, target_src_full_mask, target_dst, target_dst_full_mask] )
