...
#test_import()
#mark=(x[-1]*255).astype('uint8')

mask_np=x[-1][:,:,:,np.newaxis]
mask=K.variable(mask_np)

aa=gaussian_blur(max(1, K.int_shape(mask)[1] // 64))(mask)
mask_val=K.eval(aa)[:,:,0]

mask2_val=(mask_val*255).astype('uint8')

cv2.imwrite('test_tmp_mark2.png',(mask2_val))
...
cv2.imwrite('test_tmp_mark.png',(mark))
